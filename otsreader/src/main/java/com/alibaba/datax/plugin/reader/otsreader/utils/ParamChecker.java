...
public class ParamChecker {
    ...
    public static PrimaryKey checkInputPrimaryKeyAndGet(TableMeta meta, List<PrimaryKeyValue> range) {
        ...
        for (Entry<String, PrimaryKeyType> e : meta.getPrimaryKeyMap().entrySet()) {
            ...
            if (e.getValue() != value.getType() && value != PrimaryKeyValue.INF_MIN && value != PrimaryKeyValue.INF_MAX) {
                ...
                primaryKeys.add(new PrimaryKeyColumn(e.getKey(), value));
            }
            i++;
        }
    ...
    public static OTSRange checkRangeAndGet(TableMeta meta, List<PrimaryKeyValue> begin, List<PrimaryKeyValue> end) {
        ...
        if (begin.size() == 0 && end.size() == 0) {
            List<PrimaryKeyColumn> beginPKs = new ArrayList<>();
            List<PrimaryKeyColumn> endPKs = new ArrayList<>();
            for (String name : meta.getPrimaryKeyMap().keySet()) {
                beginPKs.add(new PrimaryKeyColumn(name, PrimaryKeyValue.INF_MIN));
                endPKs.add(new PrimaryKeyColumn(name, PrimaryKeyValue.INF_MAX));
            }
            PrimaryKey beginRow = new PrimaryKey(beginPKs);
            PrimaryKey endRow = new PrimaryKey(endPKs);
            range.setBegin(beginRow);
            range.setEnd(endRow);
        } else {
            PrimaryKey beginRow = checkInputPrimaryKeyAndGet(meta, begin);
            PrimaryKey endRow = checkInputPrimaryKeyAndGet(meta, end);
            range.setBegin(beginRow);
            range.setEnd(endRow);
        }
        return range;
    ...
    public static Direction checkDirectionAndEnd(TableMeta meta, PrimaryKey begin, PrimaryKey end) {
    ...
    public static void checkInputSplitPoints(TableMeta meta, OTSRange range, Direction direction, List<PrimaryKeyValue> points) {
        ...
        PrimaryKeyValue begin = range.getBegin().getPrimaryKeyColumn(part.getName()).getValue();
        PrimaryKeyValue end = range.getEnd().getPrimaryKeyColumn(part.getName()).getValue();
